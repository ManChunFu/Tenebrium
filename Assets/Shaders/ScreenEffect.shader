    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _EmotionSlider)
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float scanLineWorld = lerp(0.0,sin((_ScanlineScale_var*(i.uv0.g+(time.r*_AnimationStrength_var)))),_ScanlineStrength_var);
...
////// Lighting:
                ...
                float node_3515 = 0.0;
                float tileSpace = 3.0;
                float _EmotionSlider_var = UNITY_ACCESS_INSTANCED_PROP( Props, _EmotionSlider );
                float node_5921 = ceil(_EmotionSlider_var);
                float2 uvTile_tc_rcp = float2(1.0,1.0)/float2( tileSpace, 3.0 );
                float uvTile_ty = floor(node_5921 * uvTile_tc_rcp.x);
                float uvTile_tx = node_5921 - tileSpace * uvTile_ty;
                float2 uvTile = (i.uv0 + float2(uvTile_tx, uvTile_ty)) * uvTile_tc_rcp;
                float4 _TextureMap = tex2D(_Tilemap,TRANSFORM_TEX(uvTile, _Tilemap));
                float4 _Colour_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Colour );
                float3 node_1485 = (_TextureMap.rgb*_Colour_var.rgb*ambientOcclusion);
